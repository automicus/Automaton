...
class EventStream(socket.socket):
    ...
    def __init__(self, parent):
        ...
        auth_data = {'user': self.parent.conn._username, 'passwd': self.parent.conn._password}
        self.data = {}
        self.data['auth'] = base64.encodestring('{user}:{passwd}'.format(**auth_data)).strip()
    ...
    def connect(self):
        if not self._connected:
            super(EventStream, self).connect((self.data['addr'], self.data['port']))
            self.setblocking(0)
