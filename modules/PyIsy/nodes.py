...
class nodes(object):
    ...
    def parse(self, xml):
        ...
        try:
            ...
            self.parent.log.error(
                'ISY Could not parse nodes, poorly formatted XML.')
        else:
            # get nodes
            ...
            for ntype in ntypes:
                ...
                for feature in features:
                    ...
                    if ntype == 'folder':
                        ...
                        self.insert(
                            nid, nname, nparent, node(self, nid, nval), ntype)
                    elif ntype == 'group':
                        self.insert(
                            nid, nname, nparent, group(self, nid), ntype)

            self.parent.log.debug('ISY Loaded Nodes')
    ...
    def getByInd(self, i):
        ...
        if self.ntypes[i] == 'folder':
            return nodes(self.nids[i], self.parent, self.nids, self.nnames,
                         self.nparents, self.nobjs, self.ntypes)
        else:
            return self.nobjs[i]

    def update(self, wait=0):
        if not self.parent.auto_update:
            for obj in self.nobjs:
                if obj.__name__ == 'node':
                    obj.update(wait)
        ...
        if not self.parent.parent.auto_update and not self.noupdate:
            ...
            if xml is not None:
                try:
                    ...
                    self.parent.parent.log.error(
                        'ISY Could not parse nodes, poorly formatted XML.')
                else:
                    self.set('status',
                             xmldoc.getElementsByTagName('property')[0].attributes['value'].value)
                    self.parent.parent.log.debug(
                        'ISY updated node: ' + self._id)
            else:
